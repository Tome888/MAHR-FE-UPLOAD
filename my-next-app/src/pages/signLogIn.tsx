...
function signLogIn() {
  ...
  const signUpFunction = (e: any) => {
    ...
    if (pass === rePass && name.trim() && email.trim() && lastName.trim()) {
      fetch("https://mahr-be-upload.onrender.com/api/user", {
        method: "POST",
        headers: {
  ...
  const loginFunc = (e: any) => {
    ...
    if (loginEmail !== "" && loginPass !== "") {
      fetch("https://mahr-be-upload.onrender.com/api/signin", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const getUserData = (email: any) => {
    fetch(
      `https://mahr-be-upload.onrender.com/api/userData?email=${encodeURIComponent(
        email
      )}`
    )
      .then((res) => {
